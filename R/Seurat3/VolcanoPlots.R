...
for(k in 1:2){
        ...
        for(i in seq_along(dates)){
                dataset[[i]] = read.csv(file = paste0("Yang/6. Differential analysis/",cell.type[k],
                                                      "/",cell.type[k],"_markers_",dates[i],"_RNA.csv"),
                                        row.names = 1,stringsAsFactors=F)
                dataset[[i]]$cluster = cluster[i]
                dataset[[i]]$gene = rownames(dataset[[i]])
                jpeg(paste0(path,"Volcano_plot_",cell.type[k],"_",cluster[i],"_RNA.jpeg"), units="in", width=10, height=7,res=600)
