...
(load(file = "./output/20181231/g1_11_20181231.Rda"))
...
save(g2,file = paste0(path,"g2_11_20181231.Rda"))
...
PCHeatmap(object, pc.use = c(1:3,35:40), cells.use = 500, do.balanced = TRUE)
dev.off()

GC()
system.time({
    object %<>% RunTSNE(reduction.use = "pca", dims.use = 1:37, do.fast = TRUE)
    object %<>% FindClusters(reduction.type = "pca", resolution = 0.6, dims.use = 1:37,
...
system.time(object %<>% RunHarmony("orig.ident", dims.use = 1:37,
...
DimHeatmap(object = object, reduction.type = "harmony", cells.use = 500, 
           dim.use = c(1:3,45:50), do.balanced = TRUE)
dev.off()
